# 가위 바위 보 게임 클래스로 구현하기
# 사용자로부터 가위, 바위, 보 중 하나를 입력받아 컴퓨터와 대결하는 게임을 만드세요.
# 사용자가 입력한 값과 컴퓨터가 랜덤으로 선택한 값을 비교하여 승패를 결정하고 결과를 출력하세요.
# 게임 승패를 판단 및 결과 출력
# 가위는 1, 바위는 2, 보는 3으로 표현
# 게임이 끝나면 계속 할 지 물어본다.
import random
class GBVGame:
    choices = {1: '가위', 2: '바위', 3: '보'}  # 클래스 변수

    def __init__(self):
        self.user_choice = None  # 인스턴스 변수
        self.computer_choice = None  # 인스턴스 변수

    def get_user_choice(self):
        while True:
            try:
                choice = int(input("가위(1), 바위(2), 보(3) 중 하나를 선택하세요: "))
                if choice in self.choices:
                    self.user_choice = choice
                    break
                else:
                    print("잘못된 입력입니다. 1, 2, 3 중 하나를 선택하세요.")
            except ValueError:
                print("숫자를 입력하세요.")
    
    def get_computer_choice(self):
        self.computer_choice = random.randint(1, 3)
    
    def determine_winner(self):
        if self.user_choice == self.computer_choice:
            return "무승부"
        elif (self.user_choice == 1 and self.computer_choice == 3) or \
             (self.user_choice == 2 and self.computer_choice == 1) or \
             (self.user_choice == 3 and self.computer_choice == 2):
            return "사용자 승리"
        else:
            return "컴퓨터 승리"

    def play(self):
        self.get_user_choice()
        self.get_computer_choice()
        print(f"사용자 선택: {self.choices[self.user_choice]}")
        print(f"컴퓨터 선택: {self.choices[self.computer_choice]}")
        result = self.determine_winner()
        print(f"결과: {result}")

# a = GBVGame()
# a.play()
# -> 객체가 1개만 있으면 되니깐 굳이 인스턴스 생성 할 필요 없이 바로 클래스를 객체로 삼아 사용
# 체이닝 기법, 객체는 별도로 없음
# return.self
GBVGame().play()

# 게임 종료 후에 사용자에게 게임을 다시 할지 물어본다.